...
pygame.display.set_caption("AutoBall Battle")
#字体和文字常量
myfont=pygame.font.Font("C:/Windows/Fonts/arial.ttf",20)
title_font=pygame.font.Font("C:/Windows/Fonts/arial.ttf",40)
myfs=pygame.font.Font("C:/Windows/Fonts/arial.ttf",15)
rank_title=myfs.render("Rank",True,(255,125,0))
#颜色常量
GRAY100=100,100,100
GRAY150=150,150,150
GRAY200=200,200,200
    ...
    def __init__(self,x,y,name=""):
        ...
        if name=="":
            try: #防止名单中名字不够用
                ...
                self.name="User"+str(random.randint(0,100))
        else:
            self.name=name
...
class Player(Ball): #玩家
    def __init__(self):
        ...
        self.color=0,0,255
        self.speed=1
        self.nametag=myfont.render("Player",True,BLACK)
        self.nametag1=myfs.render("Player",True,BLACK)
...
def display_kill(ball1,ball2):
    ...
    if ball1.type in ["PlayerPart","Player"] and ball2.type in ["PlayerPart","Player"]:
        return
    str1=ball1.name+" ate "+ball2.name
...
if "auto" in sys.argv:
    ...
    player=Player()
balls=[AIPlayer(1100,1100,"BestFriend"),player]
for i in range(13): #添加球
    balls.append(AIPlayer(random.randint(0,width),random.randint(0,height)))

#显示帮助
helptext=[
    myfont.render("AutoBall Battle - V0.0.3",True,GREEN1),
    myfont.render("Use ←→ arrow keys to adjust moving direction;",True,BLACK),
    myfont.render("Use W,A,S,D keys to switch up,left,down,right direction;",True,BLACK),
    myfont.render("Use tab to hide or display player moving direction;",True,BLACK),
    myfont.render("Use space to split.",True,BLACK),
    myfont.render("",True,BLACK),
    myfont.render("Maker:BuggyPlayer  Sorry for my bad English.",True,BLACK)
]
def cancelhelp():
    global showhelp
    time.sleep(5)
